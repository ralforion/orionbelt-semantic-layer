...
- **The json group of the portable function catalog: `json_value(x, path)`.** The scalar at a literal JSONPath, as a string, NULL when the path is absent or resolves to an object rather than a scalar. Every rendering was measured against a live engine before it was written, and two of them contradict the obvious choice: DuckDB has its own `JSON_VALUE` but it returns the value still quoted (`'"x"'`), so it renders as `json_extract_string`, and ClickHouse returns the **empty string** rather than NULL for an absent path, so the call is wrapped in `nullIf(..., '')` - which restores NULL for the common case but cannot distinguish an absent path from a genuine empty-string value, and says so. The path is pinned to a literal because the engines do not merely spell the call differently, they take the path apart differently: Postgres wants the segments as separate arguments to `json_extract_path_text`, Snowflake wants them dotted without the `$`, and BigQuery, Databricks and MySQL take the JSONPath verbatim. A non-literal path falls through to the pass-through path rather than compiling to a call the engine refuses. The accepted subset is object member access and array subscripts rooted at `$`; filters and wildcards are left out because the engines diverge on them and a catalog entry has to pin one meaning. `json_query` and `json_exists` were measured alongside it and deliberately **not** admitted: `json_exists` is missing on BigQuery, Postgres and Snowflake, and `json_query` is missing on two engines and array-wraps its result on ClickHouse (`[{"b":"y"}]` where BigQuery gives `{"b":"y"}`), so neither has one meaning to pin. Dremio is the **first dialect to declare an `unsupported_functions` entry**: it reaches into semi-structured data through field access on a parsed column rather than a JSONPath scalar function, so it reports the call unsupported instead of mis-rendering it. Every example is executed on DuckDB, BigQuery, Postgres, ClickHouse and Snowflake and asserted against the documented value; the Databricks and MySQL renderings are derived from their published references, the Databricks SQL warehouse having refused to start while the group was measured.
