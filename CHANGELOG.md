...
- [#83](https://github.com/ralfbecher/orionbelt-semantic-layer/issues/83) Resolver silently drops `model.name` / `model.description` / `dataObject.description`.
- [#84](https://github.com/ralfbecher/orionbelt-semantic-layer/issues/84) Spec ↔ ontology ↔ RDF exporter are three drifting contracts (bidirectional drift guard needed).
- [#85](https://github.com/ralfbecher/orionbelt-semantic-layer/issues/85) JSON schemas in `schema/` have concrete drift vs the Pydantic models (root `additionalProperties` mis-nested, removed fields advertised, missing `grouping`).
- [#86](https://github.com/ralfbecher/orionbelt-semantic-layer/issues/86) Public ChatGPT Action OpenAPI on v2.5.0 with old `dimension`/`operator` query keys; OBML reference missing 3 of the 8 dialects. Already tracked by an `xfail(strict=True)` in `test_notebook_contracts.py` — fixing #86 will auto-flip the test to pass and force removal of the marker.
