...
- **Grain deduplication refused queries over features it does not actually conflict with.** The compatibility rule keyed on whether a query contained a `total: true` measure *anywhere*, so asking for a base-grain total alongside a deduplicated measure raised a fanout error even though the two never interact - the totals pass runs after deduplication and wraps its output in a `base` CTE, adding `AGG(x) OVER ()` around a column the dedup CTE already finished computing. Only a `total` on a measure that is itself deduplicated conflicts, and that is still refused. The remaining combinations stay blocked: `filterContext` emits its own CTE named `main` and would collide, period-over-period rebuilds the FROM clause from a date spine instead of wrapping the incoming AST, and cumulative/window are unverified rather than known-safe.
