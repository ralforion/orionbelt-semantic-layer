...
- **Grain deduplication refused queries over features it does not actually conflict with.** The compatibility rule keyed on whether a query contained a `total: true` measure *anywhere*, so asking for a base-grain total alongside a deduplicated measure raised a fanout error even though the two never interact - the totals pass runs after deduplication and wraps its output in a `base` CTE, adding `AGG(x) OVER ()` around a column the dedup CTE already finished computing. Only a `total` on a measure that is itself deduplicated conflicts, and that is still refused. The remaining combinations stay blocked, each confirmed by letting it through and executing the result: cumulative and window re-project the measure's raw aggregate instead of selecting it by alias, emitting `SUM("Sales"."quantity")` into a CTE whose FROM is only the dedup output (`Referenced table "Sales" not found`); `filterContext` emits its own CTE named `main` (`Duplicate CTE name "main"`); and period-over-period rebuilds the FROM from a date spine, re-joining tables the dedup CTEs already joined (`Ambiguous reference to table ... duplicate alias`). Alias reference composes, expression re-projection does not - which is why `total` works and the others do not.
