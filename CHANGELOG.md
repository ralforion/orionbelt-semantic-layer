...
- **ClickHouse no longer answers with a wrapped or saturated integer when a cast overflows.** A measure declaring `dataType: integer` compiled to `CAST(SUM(x) AS Nullable(Int32))`, and ClickHouse neither raises nor returns the value there: a true 4000000000 came back as **-294967296**, wrapped through two's complement, and a Float64 source saturates to 2147483647 instead. Every other engine either raises or holds the value. This is the defect class #336 named, on a different engine and a different type, and the #336 fix does not reach it: that one widened MySQL's decimal casts. The integer path now renders `accurateCast`, which raises on overflow where `CAST` does not. `accurateCast` will not take a fraction, so the value goes through an exact scale-0 decimal first, which truncates it exactly as the plain `CAST` already did and leaves every non-overflowing answer unchanged. The intermediate is `toDecimal256OrNull(toString(x), 0)` rather than the obvious `trunc(x)`, for three reasons: `trunc` refuses a `String`, so a measure aggregating a text column under `dataType: integer` raised where it used to answer; scale 0 discards a fraction that is being discarded anyway and moves the `Decimal256` ceiling out to 76 digits, so 1e60 raises rather than silently reading NULL; and the round trip is exact, `toString` of a `Float64` being its shortest round-tripping form and a `Decimal` carried digit for digit, verified against the plain `CAST` on a `Decimal(38, 3)` and at both 2^53+1 and `Int64` max. Past 76 digits the conversion yields NULL rather than raising, which the "value, NULL, or refusal" contract allows, and it costs one conversion per output group rather than per row, every caller wrapping an aggregate. A NULL literal and a whole-number literal are both exempt, the second because every CFL count pad is one and converting it is noise. Reachable only with the default float `resultType`: declaring `resultType: int` casts the SUM argument to `Decimal128` under the #338 fix, and a decimal-to-integer cast already raised.
