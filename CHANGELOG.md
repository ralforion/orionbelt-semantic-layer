...
- **A metric referencing another metric compiled to SQL no engine can bind.** The planner substitutes a metric's components one level only, so `Quadrupled: '{[Doubled]} * 2'` emitted the inner metric's own placeholder as a bare column: `CAST("Sold Quantity" * 2 * 2 AS DECIMAL(18, 2))` over a FROM that has no such column, failing at execution with `Referenced column "Sold Quantity" not found in FROM clause`. Same for a derived metric over a cumulative or period-over-period metric, whose wrappers only run when the metric carrying the feature is itself selected. Nesting is now rejected at model load with `UNSUPPORTED_METRIC_REF`, naming both metrics and pointing at the underlying measures, so `obsl validate` reports it with a source span instead of the query failing in the warehouse. One composition is unaffected because the compiler does expand it: a derived metric over a **window** metric (`'{[Revenue]} - {[Revenue Prior Month]}'`), which the window wrapper serves by projecting the window metric's base measure as a column of its base CTE. Metric `measure:` references were already validated against measures only and are unchanged.
