...
- **`to_number(x)` is in the portable function catalog (#375).** The other half of `cast`, and the half a value read out of JSON wants: `json_value` is specified to return a string, and a cast over text is answered differently by every engine - `cast('abc', 'double')` raises on five, is NULL on ClickHouse, and is **0** on MySQL, which is a plausible wrong number rather than a failure. **Text that does not name a number is NULL, on all eight dialects.** Five have a form that says so - `TRY_CAST` on DuckDB, Snowflake, Databricks and Dremio, `SAFE_CAST` on BigQuery, `toFloat64OrNull` on ClickHouse. PostgreSQL, MySQL and Dremio have none at any version, so on those three the text is tested against a numeric pattern *before* it is converted; testing afterwards is not available on MySQL, whose failure is a silent 0 that nothing downstream can tell from a genuine zero. Surrounding whitespace is ignored, because the engines split on it: `' 42 '` is 42 to DuckDB's `TRY_CAST` and NULL to ClickHouse's `toFloat64OrNull`, so the argument is trimmed and the answer is 42 everywhere.
