...
- **Synthesized counts**: every countable `dataObject` yields a grain-anchored row-count measure whose **name == label** (default `"Sales Count"`, like a declared `"Order Count"`) — an anchored `COUNT(*)`, integer-typed. Counts are **named measures — no ad-hoc aggregation in queries**; reference `"Sales Count"` in `select.measures` like any measure. Name/label resolution (D5 precedence): per-object `countLabel` > model `countLabelPattern` (only the `{object}` token, interpolates the object's display label) > default `"{object} Count"`. Knobs: per-object `countable: false` (opt-out); model-level `exposeCounts: false`. A declared measure of the same name overrides synthesis (e.g. `COUNT(DISTINCT pk)`). Synthesized counts are computed via `SemanticModel.effective_measures` (declared win) and are never persisted — they don't roundtrip through YAML/OSI (the knobs do, via `synthesis.py`). A `dataObject` is never a queryable FROM target.
