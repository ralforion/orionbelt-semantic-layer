...
_NUMERIC_FUNCTIONS: tuple[FunctionSpec, ...] = (
    ...
    FunctionSpec(
        ...
        semantics=(
            ...
            "across eight engines, seven agreed and ClickHouse did not: its "
            "``round`` takes a *float's* ties to even, so 2.5 cast to "
            "``decimal(18, 0)`` came back 2 and -2.5 came back -2 where every "
            "other engine said 3 and -3. It rounds a *Decimal's* ties away "
            "from zero, so the call converts to an exact decimal first - the "
            "same move ``round`` makes on PostgreSQL, handing the engine the "
            "type it already rounds correctly.\n\n"
            "**One exception, on ClickHouse alone: the rounding reads one "
            "digit more than the value it rounds, and Decimal256 holds 76 of "
            "them.** Where the value and that digit do not both fit, the "
            "conversion truncates. Two shapes reach it: the "
            "``decimal(76, s)`` ceiling, whose own width spends every digit - "
            "measured, 2.555 answers 2.56 at ``decimal(75, 2)`` and 2.55 at "
            "``decimal(76, 2)`` - and a value wide enough to reach the same "
            "limit under a narrower target, where at ``decimal(75, 2)`` 73 "
            "integer digits and .555 round to .56 and 74 truncate to .55. "
            "Neither is reachable from a number this engine holds: 77 "
            "significant digits arrive as text. A model that wants the "
            "rounding at the ceiling declares one digit less, and that costs "
            "no range - both widths are stored in the same 256-bit integer, "
            "and a value in the digit the engine carries beyond what a "
            "decimal declares is truncated rather than refused.\n\n"
            "The conversion is also what lets this engine take a *text* "
            "argument at all: ``round('4.6', 2)`` raises there, so a cast over "
            "anything string-shaped - a ``json_value``, which is specified to "
            "return a string - did not compile to something it would run.\n\n"
