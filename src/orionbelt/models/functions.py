...
_NUMERIC_FUNCTIONS: tuple[FunctionSpec, ...] = (
    ...
    FunctionSpec(
        ...
        semantics=(
            ...
            "exact-decimal cast on PostgreSQL and MySQL - ``CAST(x AS "
            "numeric)``, ``CAST(x AS DECIMAL(65, s))`` - so the native ROUND "
            "sees the type it already rounds correctly. The cast keeps 18 "
            "fractional digits, or the digit count asked for when that is "
            "larger, so it never drops the digit being rounded to. ClickHouse "
            "converts nothing, because there the conversion is the lossy step: "
            "``toDecimal256`` scales a Float64 by a power of ten in floating "
            "point, so ``round(toDecimal256(1e19, 18))`` is 9999999999999999539 "
            "and an infinity cannot be converted at all. It gets arithmetic "
            "whose only typed part is the half, which preserves a Decimal and "
            "a Float64 alike. DuckDB, BigQuery, Snowflake, Databricks and "
            "Dremio need no rewrite.\n\n"
