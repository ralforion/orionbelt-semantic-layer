...
_NUMERIC_FUNCTIONS: tuple[FunctionSpec, ...] = (
    ...
    FunctionSpec(
        ...
        semantics=(
            ...
            "``toFloat64OrNull`` on ClickHouse. PostgreSQL, MySQL and Dremio "
            "have none at any version, so on those three the text is tested "
            "against a numeric pattern *before* it is converted. Testing "
            "after is not available on MySQL: its failure is a silent 0, and "
            "no guard can tell that from a genuine zero once it has "
            "happened.\n\n"
