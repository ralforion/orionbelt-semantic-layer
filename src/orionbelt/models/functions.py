...
_NUMERIC_FUNCTIONS: tuple[FunctionSpec, ...] = (
    ...
    FunctionSpec(
        ...
        semantics=(
            ...
            "DECIMAL(65, s))``, ``toDecimal256(x, s)`` - so the native ROUND "
            "sees the type it already rounds correctly. The cast keeps 18 "
            "fractional digits, or the digit count asked for when that is "
            "larger, so it never drops the digit being rounded to. DuckDB, "
            "BigQuery, Snowflake, Databricks and Dremio need no rewrite.\n\n"
