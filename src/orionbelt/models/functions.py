...
_NUMERIC_FUNCTIONS: tuple[FunctionSpec, ...] = (
    ...
    FunctionSpec(
        ...
        semantics=(
            ...
            "**One exception, on ClickHouse alone: a target at that engine's "
            "precision ceiling truncates instead.** The rewrite converts "
            "through an intermediate one place wider than the target so the "
            "rounding has a digit to read, and Decimal256 is 76 digits "
            "wherever the point sits, so at ``decimal(76, s)`` that place and "
            "the target's own integer width cannot both exist. Measured: "
            "2.555 answers 2.56 at ``decimal(75, 2)`` and 2.55 at "
            "``decimal(76, 2)``. A model that wants the rounding declares one "
            "digit less, and that costs no range: both widths are stored in "
            "the same 256-bit integer, and a value in the digit the engine "
            "carries beyond what a decimal declares is truncated rather than "
            "refused.\n\n"
