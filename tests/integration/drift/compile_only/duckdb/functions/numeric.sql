...
-- to_number(x)
--   to_number('4.6') = 4.6
CASE WHEN regexp_matches(TRIM(CAST('4.6' AS VARCHAR)), '^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$') THEN TRY_CAST(TRIM(CAST('4.6' AS VARCHAR)) AS DOUBLE) END;
--   to_number('abc') = None
CASE WHEN regexp_matches(TRIM(CAST('abc' AS VARCHAR)), '^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$') THEN TRY_CAST(TRIM(CAST('abc' AS VARCHAR)) AS DOUBLE) END;
