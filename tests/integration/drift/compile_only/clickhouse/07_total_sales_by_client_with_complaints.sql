...
SELECT "Clients"."clientname" AS "Sales Client Name", CAST(NULL AS Nullable(String)) AS "Complaint Client Name", CAST(round(toDecimal256(toString("Sales"."salesamount"), 20), 20) AS Nullable(Decimal(76, 20))) AS "Total Sales", CAST(NULL AS Nullable(Int32)) AS "Client Complaints Count"
...
SELECT COALESCE("Sales Client Name", "Complaint Client Name") AS "Client", CAST(round(toDecimal256(toString(SUM("composite_01"."Total Sales")), 3), 2) AS Nullable(Decimal(18, 2))) AS "Total Sales", accurateCast(trunc(COUNT("composite_01"."Client Complaints Count")), 'Nullable(Int32)') AS "Client Complaints Count"
