...
SELECT `Sales`.`salesamount` AS `Total Sales`, CAST(NULL AS DECIMAL(18, 2)) AS `Total Purchases`
FROM ``.`orionbelt_1`.`sales` AS `Sales`
UNION ALL
SELECT CAST(NULL AS DECIMAL(18, 2)) AS `Total Sales`, `Purchases`.`purchaseamount` AS `Total Purchases`
