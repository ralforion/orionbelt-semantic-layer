...
SELECT ROUND(CAST(SUM(`composite_01`.`Total Returns`) AS NUMERIC), 2) AS `Total Returns`, ROUND(CAST(SUM(`composite_01`.`Total Sales`) AS NUMERIC), 2) AS `Total Sales`, ROUND(CAST((SUM(`composite_01`.`Total Returns`) / NULLIF(SUM(`composite_01`.`Total Sales`), 0)) AS NUMERIC), 4) AS `Return Rate`
