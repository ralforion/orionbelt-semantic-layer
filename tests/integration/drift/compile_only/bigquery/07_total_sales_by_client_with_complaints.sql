...
SELECT `Clients`.`clientname` AS `Sales Client Name`, CAST(NULL AS STRING) AS `Complaint Client Name`, `Sales`.`salesamount` AS `Total Sales`, CAST(NULL AS STRING) AS `Complaint Count`
