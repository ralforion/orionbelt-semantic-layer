-- Q65 — OBSL-compiled, dialect: duckdb
...
WITH "base" AS (
  SELECT
    ...
    COUNT(
      DISTINCT CASE WHEN NOT "Store Sales"."ss_sales_price" IS NULL THEN "Item"."i_item_sk" END
  ...
  WHERE
    ...
    AND NOT "Store Sales"."ss_item_sk" IS NULL
  GROUP BY ALL
), "having_window" AS (
  SELECT
