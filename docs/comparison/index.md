...
| Transport security on the self-hosted query surface | ✅ **TLS on both wire surfaces** (v2.28.0+): pgwire answers the `SSLRequest` and upgrades the socket, Flight SQL serves `grpc+tls`, and both take a client CA for **mutual TLS**. Verified against libpq, Python ADBC, the Flight SQL JDBC driver and DuckDB | n/a — no self-hosted runtime; dbt Cloud terminates TLS at its edge | n/a for a wire protocol — the server surface is the **Publisher** companion (REST + MCP), not a SQL listener, so there is no `SSLRequest` to answer; Publisher's own HTTPS posture is not assessed here | n/a — no SQL wire listener; the API and JDBC driver are HTTPS | ❌ on the listener: the SQL API answers `N` to every `SSLRequest` ([`pg-srv/src/protocol.rs`](https://github.com/cube-js/cube/blob/master/rust/cubesql/pg-srv/src/protocol.rs) `SSLResponse` is `const CODE: u8 = b'N'`). Cube Cloud terminates TLS at its edge | ✅ TLS on ODBC/JDBC (SQL) and XMLA (MDX) |
