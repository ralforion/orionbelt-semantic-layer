...
| Portable scalar-function catalog in expressions | ✅ **40 entries** (string, numeric, conditional, date/time, JSON). The *answer* is pinned, not just the spelling — see [below](#what-portable-means-here) | ✅ cross-database macros (`dateadd`, `datediff`, `split_part`, `listagg`, …) | ✅ standard function set, implemented per dialect | ❌ native SQL per dialect | ❌ raw SQL passthrough | MDX function layer |
