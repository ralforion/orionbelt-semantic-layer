...
| Portable scalar-function catalog in expressions | ✅ **39 entries** in 5 groups (string, numeric, conditional, date/time, json). Pinned where engines disagree on the *answer* rather than the spelling: `round(2.5)` is 3 everywhere including ClickHouse, whose ties go to even and whose call is rewritten arithmetically; `trunc(-1.9)` is -1 including Databricks, which has no numeric truncation; `greatest(1, NULL, 3)` is NULL everywhere. Every entry carries **executed** examples asserted against live engines, and `expressionMode: portable` turns an uncatalogued call into an error rather than vendor SQL | ✅ cross-database macros (`dateadd`, `datediff`, `split_part`, `listagg`, …) | ✅ standard function set, implemented per dialect | ❌ native SQL per dialect | ❌ raw SQL passthrough | MDX function layer |
| JSON / semi-structured access in expressions | ✅ `json_value(x, path)` on all 8 dialects, with the contract **enforced rather than inherited**: NULL for an absent path *and* for one resolving to an object or array, which four engines otherwise answer with serialized JSON and are wrapped in a type guard for. Path pinned to a literal because engines take it apart differently, not merely spell it differently | Via warehouse SQL | Via warehouse SQL | Via warehouse SQL | Via warehouse SQL | Via warehouse SQL |
| Conversion / funnel metrics | ❌ | ✅ | Patterns | Patterns | Patterns | Patterns |
| Cross-engine numeric contract (zero divisor, overflow, exact integer `AVG`/`SUM`) | ✅ pinned per engine and **verified by execution**: a zero divisor is NULL everywhere rather than `inf` on one engine and an error on four; a cast that outgrows its type never returns a saturated number; an integer `AVG` or `SUM` is exact on the engines whose aggregate drifts or wraps | Inherits engine semantics | Inherits engine semantics | Inherits engine semantics | Inherits engine semantics | Inherits engine semantics |
