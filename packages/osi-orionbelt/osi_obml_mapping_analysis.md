...
The converter restructures between these two representations automatically, preserving column mappings and generating descriptive relationship names. The OSI schema has no cardinality field, so the OBML join cardinality (`joinType`) is **inferred** from the datasets' declared keys: `to_columns` covering the `to` dataset's `primary_key`/`unique_keys` (with `from_columns` covering the `from` dataset's) yields `one-to-one`, `to_columns` alone covering yields `many-to-one`, a target with keys that the FK does not reference yields `many-to-many` (fan-out), and a target with no declared keys falls back to `many-to-one` - OSI's own schema describes `from` as the many side and `to` as the one side. (OBML has no `one-to-many`; a one-to-many A→B is a many-to-one B→A.)
...
| Aggregate nested in a larger expression (`ROUND(SUM(a.x), 2)`, `CASE WHEN SUM(a.x) > 0 THEN SUM(a.y) END`) | Auto-generated measures + metric formula |

The decomposition is driven by **sqlglot**: the resolved SQL is parsed under its source dialect grammar, the aggregate nodes are located in the AST, and each is lifted into an auto-measure with the surrounding expression becoming the metric formula. This is why an aggregate nested inside `ROUND(...)`, a `CASE` expression, or any other function decomposes rather than falling back to preserve-verbatim, and why a numeric or string literal containing a dot (`1.23`, `'north.1'`) is never mistaken for a `dataset.column` reference.
